...
export const GET = withErrorHandling(async (request) => {
  ...
  const [orders, invoices, agreements] = await Promise.all([
    ...
    q(env.DB, `SELECT a.id,a.agreement_number,a.status,a.expires_at,a.accepted_at,a.rendered_html,a.unsigned_pdf_document_id,a.signed_pdf_document_id,o.order_number FROM agreements a JOIN orders o ON o.id=a.order_id WHERE a.customer_id = ? ORDER BY a.created_at DESC LIMIT 20`, ctx.portalCustomerId),
  ]);
