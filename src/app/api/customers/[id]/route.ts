...
export const GET = withErrorHandling<{ params: Promise<{ id: string }> }>(async (request, context) => {
  ...
  const [orders, invoices, agreements, history] = await Promise.all([
    ...
    q(env.DB, "SELECT invoice_number,status,due_date,total_cents,balance_due_cents,order_id,created_at FROM invoices WHERE customer_id=? ORDER BY created_at DESC", customerId),
    q(env.DB, "SELECT agreement_number,status,order_id,accepted_at,expires_at,verification_code,created_at FROM agreements WHERE customer_id=? ORDER BY created_at DESC", customerId),
  ...
  return Response.json({ customer, orders, invoices, agreements, history });
});
