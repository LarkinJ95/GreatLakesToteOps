...
type Action = "stage" | "load" | "deliver" | "pickup";
const actionMeta: Record<Action, { label: string; statuses: string[] }> = {
  stage: { label: "Stage all equipment", statuses: ["reserved"] },
  load: { label: "Load all equipment", statuses: ["staged"] },
  deliver: { label: "Deliver all equipment", statuses: ["loaded"] },
  pickup: { label: "Pick up all equipment", statuses: ["delivered", "rented", "pickup_scheduled"] },
};
...
export const POST = withErrorHandling<{ params: Promise<{ id: string }> }>(
  async (request, context) => {
    ...
    const action = (["stage", "load", "deliver", "pickup"] as Action[]).find((candidate) =>
    ...
    for (const asset of targets) {
      const result = await processScan(env.DB, {
        ...
        assetIdentifier: asset.qr_code_value, mode: action, orderId,
