...
export function Book() {
  ...
  const [serverQuote, setServerQuote] = useState<
    (PriceSummary & { quoteId?: string }) | null
  >(null);
  ...
  function addressBlock(
    ...
    return (
      <fieldset className="rounded-xl border border-border p-4">
        <legend className="px-2 text-sm font-bold text-navy-700">
          {label}
        </legend>
        <div className="grid gap-4 sm:grid-cols-[2fr_1fr_1fr]">
          <div>
            <label htmlFor={`${prefix}-street`} className={labelCls}>
              Street
            </label>
            <input
              id={`${prefix}-street`}
          ...
          <div>
            <label htmlFor={`${prefix}-city`} className={labelCls}>
              City
            </label>
            <input
              id={`${prefix}-city`}
          ...
          <div>
            <label htmlFor={`${prefix}-zip`} className={labelCls}>
              ZIP
            </label>
            <input
              id={`${prefix}-zip`}
    ...
    switch (step) {
      ...
      case 1:
        return (
          ...
          (sameAddress ||
            (!!pickup.street && !!pickup.city && pickup.zip.length === 5))
        );
      case 2:
        return !!deliveryDate && !!pickupDate && pickupDate >= deliveryDate;
      case 3:
        return (
          !!pkg && !!availability && availability.status !== "outside-area"
        );
      case 5:
        return (
          !!customer.firstName &&
      ...
      case 7:
        return agreementAccepted && signature.trim().length >= 2;
      default:
        return true;
    }
  }
    ...
    <main id="main-content">
      ...
      <section className="bg-white py-10 lg:py-14">
        <div className="container-site max-w-4xl">
            ...
            <ol className="flex flex-wrap items-center gap-1.5">
              {steps.map((label, i) => (
                <li key={label} className="flex items-center gap-1.5">
                  ...
                  <span
                    className={`hidden text-xs font-semibold sm:inline ${i === step ? "text-navy-700" : "text-charcoal-300"}`}
                  >
                    {label}
                  </span>
                  {i < steps.length - 1 && (
          ...
          {step === 0 && (
            <fieldset>
              <legend className="text-xl font-bold text-navy-700">
                What is this rental for?
              </legend>
          ...
          {step === 1 && (
            <div className="grid gap-5">
              <h2 className="text-xl font-bold text-navy-700">
                Where are we delivering and picking up?
              </h2>
              ...
              {!sameAddress &&
                addressBlock("Pickup address", pickup, setPickup, "pick")}
              <div className="grid gap-4 sm:grid-cols-3">
                <div>
                  <label htmlFor="prop-type" className={labelCls}>
                    Property type
                  </label>
                  <select
                    id="prop-type"
                ...
                <div>
                  <label htmlFor="stairs" className={labelCls}>
                    Stairs at delivery?
                  </label>
                  <select
                    id="stairs"
                  ...
                  >
                    {["No", "Yes — a few steps", "Yes — multiple flights"].map(
                      o => (
                        <option key={o}>{o}</option>
                      )
                    )}
                ...
                <div>
                  <label htmlFor="elevator" className={labelCls}>
                    Elevator available?
                  </label>
                  <select
                    id="elevator"
              ...
              <div>
                <label htmlFor="access" className={labelCls}>
                  Parking, gate, or access notes (optional)
                </label>
                <textarea
                  id="access"
                  className={inputCls}
          ...
          {step === 2 && (
            <div className="grid gap-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label htmlFor="del-date" className={labelCls}>
                    Delivery date
                  </label>
                  <input
                    id="del-date"
                ...
                <div>
                  <label htmlFor="pick-date" className={labelCls}>
                    Pickup date
                  </label>
                  <input
                    id="pick-date"
                ...
                <div>
                  <label htmlFor="del-window" className={labelCls}>
                ...
                <div>
                  <label htmlFor="pick-window" className={labelCls}>
              ...
              <p className="rounded-xl bg-mist p-4 text-sm text-charcoal-500">
                The {pkg.name} package includes a {pkg.rentalDays}-day rental.
                Longer rentals can be arranged with extra-week add-ons on the
                next steps.
              </p>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="grid gap-5">
              ...
              {checkingAvailability && (
                <div
                  role="status"
                  className="flex items-center gap-3 rounded-xl bg-mist p-5 text-navy-700"
                >
                  <Loader2 className="h-5 w-5 animate-spin" aria-hidden />
                  Checking clean inventory, route capacity, and your service
                  zone…
                </div>
              )}
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-charcoal-400">
                          {p.totes} totes · {p.dollies}{" "}
                          {p.dollies > 1 ? "dollies" : "dolly"} · {p.rentalDays}
                          -day rental
                        </p>
                      </div>
                      <p className="text-xl font-extrabold text-navy-700">
                        ${currentPrice(p)}
                      </p>
                    </button>
                  ))}
              ...
              <p className="text-xs text-charcoal-300">
                Availability is approximate until your reservation is completed
                and confirmed.
              </p>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="grid gap-5">
              ...
              <div className="grid gap-3">
                {[
                  ...addOns.filter(
                    a => a.active && a.id !== "addon-extra-week"
                  ),
                  {
                    id: "addon-extra-week",
                    name: "Extra rental week",
                    description: `Extend this package by seven days ($${pkg.extraWeekPrice}/week).`,
                    price: pkg.extraWeekPrice,
                    unit: "per week",
                    maxQty: 4,
                    active: true,
                  },
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            onClick={() =>
                              setAddOnSel({
                                ...addOnSel,
                                [a.id]: Math.max(0, qty - 1),
                              })
                            }
                            className="h-9 w-9 rounded-lg border border-border font-bold text-navy-700 hover:bg-mist"
                          >
                            −
                          </button>
                          <span
          ...
          {step === 5 && (
            <div className="grid gap-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                  <label htmlFor="promo" className={labelCls}>
            ...
            <div className="grid gap-6">
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                <dl className="grid gap-3 rounded-2xl border border-border p-5 text-sm">
                  <div>
                    <dt className="font-bold text-navy-700">Rental type</dt>
                    <dd>{rentalType}</dd>
                  </div>
                  <div>
                    <dt className="font-bold text-navy-700">Package</dt>
                    <dd>
                      {pkg.name} — {pkg.totes} totes, {pkg.dollies}{" "}
                      {pkg.dollies > 1 ? "dollies" : "dolly"}
                    </dd>
                  </div>
                  <div>
                    <dt className="font-bold text-navy-700">Delivery</dt>
                    <dd>
                      {deliveryDate} · {deliveryWindow}
                      <br />
                      {delivery.street}, {delivery.city}, MI {delivery.zip}
                    </dd>
                  </div>
                  <div>
                    <dt className="font-bold text-navy-700">Pickup</dt>
                    <dd>
                      {pickupDate} · {pickupWindow}
                      <br />
                      {effectivePickup.street}, {effectivePickup.city}, MI{" "}
                      {effectivePickup.zip}
                    </dd>
                  </div>
                  <div>
                    <dt className="font-bold text-navy-700">Access</dt>
                    <dd>
                      {propertyType} · Stairs: {stairs} · Elevator: {elevator}
                      {contactless ? " · Contactless" : ""}
                    </dd>
                  </div>
                  {zoneInfo?.zone && (
                    <div>
                      <dt className="font-bold text-navy-700">Service zone</dt>
                      <dd>
                        {zoneInfo.zone.name}
                        {zoneInfo.zone.fee > 0
                          ? ` ($${zoneInfo.zone.fee} fee)`
                          : " (delivery included)"}
                      </dd>
                    </div>
                  )}
                ...
                <div className="rounded-2xl bg-navy-700 p-5 text-white">
                  ...
                  <ul className="mt-3 grid gap-2 text-sm">
                    ...
                    <li className="flex justify-between gap-4 border-t border-white/15 pt-2">
                      <span className="text-navy-100">
                        Estimated MI sales tax (6%)
                      </span>
                      <span className="font-semibold">
                        ${price.estimatedTax.toFixed(2)}
                      </span>
                    </li>
                  ...
                  <p className="mt-3 text-xs text-navy-200">
                    Final pricing is recalculated on the server when your
                    reservation is submitted, and a pricing snapshot is stored
                    with your order.
                  </p>
                </div>
              </div>
          ...
          {step === 7 && (
            <div className="grid gap-5">
              ...
              <div className="max-h-64 overflow-y-auto rounded-xl border border-border bg-mist p-5 text-sm leading-relaxed text-charcoal-500">
                <p className="font-bold text-navy-700">
                  Rental Agreement Overview — {site.legalName}
                </p>
                <p className="mt-2">
                  By accepting, you agree that: (1) rented equipment remains the
                  property of {site.legalName}; (2) you will use totes and
                  dollies per the safe-use rules; (3) normal wear is expected,
                  but equipment damaged beyond normal use or not returned is
                  subject to the replacement fees listed on the pricing page;
                  (4) {site.legalName} rents and delivers equipment only and
                  does not transport customer belongings; (5) delivery and
                  pickup windows are scheduled and may be adjusted with notice;
                  (6) cancellations follow the posted cancellation policy; (7)
                  extensions require approval and are billed at the package
                  extension rate.
                </p>
                <p className="mt-2">
                  The full rental agreement is generated through our secure
                  agreement system and sent to your email for review before
              ...
              <label className="flex items-start gap-3 text-sm font-semibold text-navy-700">
                ...
                />
                I have reviewed and accept the rental agreement and required
                policies.
              </label>
              <div>
                <label htmlFor="sig" className={labelCls}>
          ...
          {step === 8 && (
            <div className="grid gap-5">
              ...
              <div className="grid gap-3">
                {(
                  [
                    {
                      id: "full",
                      label: `Pay in full — $${price.estimatedTotal.toFixed(2)}`,
                      desc: "Complete payment now and your reservation is confirmed.",
                    },
                    {
                      id: "deposit",
                      label: `Deposit — $${Math.max(25, price.estimatedTotal * 0.25).toFixed(2)}`,
                      desc: "Reserve your dates with a deposit; the balance is due before delivery.",
                    },
                    {
                      id: "terms",
                      label: "Approved business-account terms",
                      desc: "Monthly invoicing for approved business accounts.",
                    },
                  ] as const
                ).map(o => (
                  <label
                  ...
                  >
                    <input
                      ...
                      onChange={() => setPaymentOption(o.id)}
                      className="mt-1 h-5 w-5 text-teal"
                    />
                    <span>
                      <span className="block font-bold text-navy-700">
                        {o.label}
                      </span>
              ...
              <p className="rounded-xl bg-mist p-4 text-sm text-charcoal-500">
                Secure payment is completed through our payment provider after
                you submit — card details are never entered on this page or
                stored by us.
              </p>
            ...
            <div className="grid gap-6 text-center">
              ...
              <div>
                ...
                <p className="mt-2 text-charcoal-500">
                  ...
                  is in our system. A confirmation and the full rental agreement
                  are on the way to <strong>{customer.email}</strong>.
                </p>
              </div>
              <dl className="mx-auto grid w-full max-w-md gap-3 rounded-2xl border border-border p-5 text-left text-sm">
                ...
                <div className="flex justify-between">
                  <dt className="font-bold text-navy-700">Estimated total</dt>
                  <dd>${price.estimatedTotal.toFixed(2)}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="font-bold text-navy-700">Payment</dt>
                ...
                <div className="flex justify-between">
                  <dt className="font-bold text-navy-700">Agreement</dt>
              ...
              <div className="flex flex-wrap justify-center gap-3">
                ...
                >
                  <CalendarPlus className="h-4 w-4" aria-hidden /> Add to
                  Calendar
                </button>
                <a href={config.portalUrl} className="btn-primary">
